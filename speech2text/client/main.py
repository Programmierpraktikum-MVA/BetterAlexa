...
try:
    while True:
        ...
        if(result["match"]):
            ...
            recording_duration = 5  # Specify the duration of the recording in seconds

            audio = sd.rec(int(recording_duration * RATE), samplerate=RATE, channels=1)
            sd.wait() # Wait until recording is finished
            # i += 1
            input_path = get_path("audio_files", "input{}.wav".format(i))
            write(input_path, RATE, audio) # Save the recording to a file
            

            output_path, language = remote_whisper(input_path) # Send the recording to the server and get the response
